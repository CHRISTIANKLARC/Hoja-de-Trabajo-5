...
void venta(){
	...
	cin>>zona;
	cout<<"\t Ingrese el tipo de inmueble:\n\t\t1 -> Terreno\n\t\t2 -> Casa\n\t\t3 -> Departamento"<<endl;
	cin>>tipo;
	...
	ofstream grabarc;
	try {
		grabarc.open("VentaInmuebles.txt",ios::app);
	...
	ifstream leerarc;
	try {
		leerarc.open("VentaInmuebles.txt",ios::in);
		while (getline(leerarc, s))
			cout<<s<<endl;
		leerarc.close();
	}
	catch(exception X){
		cout<<"Error en el archivo intente de nuevo"<<endl;
	}
}
...
void reporte(){
	...
	try {
		db.open("VentaInmuebles.txt",ios::in);
