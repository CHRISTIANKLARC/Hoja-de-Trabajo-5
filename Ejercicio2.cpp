...
void menu(){
	...
	do{
		...
		if(option==1){
			int n,i=0;
			system("CLS");
			cout<<"Cuantas entradas solicitara? "<<endl;
			cin>>n;
			for(i;i<n;i++){
				system("CLS");
				input();
...
void input(){
	...
	int n=0;
	system("CLS");
	cout<<"Seleccione  localidad"<<endl;
	...
	suma=suma+price;
	n=n+1;
	cout<<"La suma por el momento es de: Q "<<suma<<endl;
	cout<<"Cantidad: "<<n<<endl;
	system("PAUSE");
	ofstream archivo;
	archivo.open("HT5_EJERCICIO_02.txt",ios::app);
	if(archivo.fail()){
		system("CLS");
		cout<<"No se puede abrir el archivo intente de nuevo"<<endl;
		exit(1);
	}
...
void read(){
	...
	ifstream archivo;
	int socio;
	cout<<"El comprador es socio seleccione una opcion?"<<endl;
	cout<<"1. Si"<<endl;
	cout<<"2. No"<<endl;
	cin>>socio;
	system("CLS");
	try{
		archivo.open("HT5_EJERCICIO_02.txt",ios::in);
		while(archivo>>t>>p){
		...
		system("CLS");
		cout<<"No se pudo abrir el archivo intente de nuevo"<<endl;
		exit(1);
	}
	system("PAUSE");
}
